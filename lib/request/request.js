...
export default new class {
  ...
  async post(url, options = {}) {
    ...
    if (options.data) {
      logger.debug(`${Log_Prefix}[Request][POST] request params data: `, options.data)
  ...
  getAgent(cf, isProxy = true) {
    ...
    // 绕过 Cloudflare Challenge 所使用的 TLS 版本，可选值：["TLSv1.1", "TLSv1.2"]，node >= 18 无法使用 TLSv1.1
    let cfTLSVersion = "TLSv1.2"
    return cf
  ...
  async _reques(options) {
    try {
      ...
      logger.debug(`${Log_Prefix}[Request][${options.method.toUpperCase()}][${_.truncate(options.url)}] response: `, res)
      return res
    } catch (err) {
